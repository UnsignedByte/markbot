...
# @Last Modified time: 2021-05-23 15:13:29
...
class Client(discord.Client):
	...
	async def on_message(self, msg):
		...
		if not msg.author.bot and msg.content.startswith(scaryprefix):
			...
			if cont == "size":
				await msg.channel.send(f"Marc using {process.memory_info().rss} bytes of memory and is {os.path.getsize('data.msgpack')} bytes in size with {len(markov)} sequences.");
			return;
